...
2026-09-12 将旧概述、旧当前状态及较早日志移至本文件，原文保留。
现状以 `handoff.md` 顶部为准。
...
- **动因**：四审报告 3 项（2 属实 + 1 文档），静态审查收敛信号明显。
- **改动**：
  1. SEC4-01 `validate_checkpoint_payload` 放行可选 `scores` 字段（检查点格式已
     含 scores，校验器不再硬崩溃；注意：当前无生产路径向该校验器喂落盘文件，
     属契约前瞻收紧）。
  2. SEC4-02 多分片续跑改为**总是**扫描合并 `shard_checkpoints/`（去掉
     `not predictions` 条件）：阶段 1 结果已入主 checkpoint 后，二次中断的
     阶段 2 分片成果仍能增量合并，不再被静默覆写。
  3. SEC4-03 `architecture.md` 架构图注：cloud/infer.py 改「单卡推理（同
     offline）」，去掉过时的「分片推理」标注。
- **验证**：171 项单测全绿 + compileall + ruff 全过。
- **下一步**：静态收敛——残余以真机冒烟为主（DINO 10 样本 / `--num-shards 2` /
  Modal 首跑），不再追加静态审查轮次。
