"""Qwen3.6-27B grounding adapter.

Qwen3.6-27B is Alibaba's 27B-parameter dense VLM (model_type ``qwen3_5``),
released 2026-04-22. It uses the same architecture as Qwen3.5-9B (hybrid
attention: 3:1 GDN/full-attention ratio) and reuses the Qwen-VL multimodal
protocol. This adapter is identical to Qwen3_5Adapter except for MODEL_NAME
and MODEL_REVISION, pointing to the HuggingFace release.

Qwen3.6-27B is deployed exclusively on Modal (H100) with direct HuggingFace
download. It requires [kernels] dependencies (flash-linear-attention,
causal-conv1d) and larger memory (48GB training / 32GB inference).
"""
...
# Qwen3.6 thinks by default before responding. The grounding protocol
...
class Qwen36_27BAdapter:
    ...
    def load(
        ...
        *,
        ...
        print(
            f"[qwen36_27b] image processor: {type(processor.image_processor).__name__}",
            flush=True,
        )
    ...
    def build_training_batch(
        ...
        *,
        ...
        if "mm_token_type_ids" not in inputs:
            raise ValueError(
                f"Qwen3.6-27B training batch is missing mm_token_type_ids for {item.get('key')}"
            )
    ...
    def collate_training_batch(self, batch: list[dict], *, processor) -> dict:
        ...
        for item in batch:
            if "mm_token_type_ids" not in item:
                raise ValueError("Qwen3.6-27B training batch is missing mm_token_type_ids")
    ...
    def prepare_inputs(self, samples: list[ModelInput]) -> dict:
        ...
        if self._processor is None:
            raise RuntimeError("Qwen36_27BAdapter.load() must run before predict()")
