...
# Pinned to the ModelScope head commit at adoption time so run fingerprints stay
# stable; SOP always loads from a local --model-path so this is identity-only.
MODEL_REVISION = "e823e888ae179eb3be02c1a48899c4f828371376"
...
# Qwen3.8 thinks by default before responding. The grounding protocol requires
# the model to emit only the box token sequence, so every chat-template call
# must disable thinking for training and inference alike.
...
class Qwen3_8Adapter:
    ...
    def load(
        ...
        *,
        ...
        print(
            f"[qwen3_8] image processor: {type(processor.image_processor).__name__}",
            flush=True,
        )
    ...
    def prepare_inputs(self, samples: list[ModelInput]) -> dict:
        ...
        if self._processor is None:
            raise RuntimeError("Qwen3_8Adapter.load() must run before predict()")
