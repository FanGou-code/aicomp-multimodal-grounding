...
## 当前状态（最后更新 2026-09-07，v5 语料 3,466 条全链完成，全量人审进行中）

- **query-foundry 规范化轮（2026-09-07）**：审查器两处实质 bug 修复——query
  编辑记录在 journal 重放时被当删除处理（人工框重启即丢）、启动播种 O(n²)
  空转（端口绑定前前端不可达）；文本 QC 裁决数据化折进组装器；模块边界与
  vendor 死代码清理；协议文档-代码提示词指纹机器校验。111 项测试全绿。
  **发现 r4 放量重组装时 head-echo 人工裁决（train 45 + val 3 条）未重放**，
  被审语料残留 48 条应改写句子；完整重放产物 `asm-*-r5-verify` 就绪（item id
  与 r4 一致，人审进度可沿用），切换待管理员定夺。详见 query-foundry handoff。
