...
离线端：平台无关的纯推理（兜底 / 免费算力）。本体只有一个脚本 `offline/infer.py`，
任何单卡 CUDA 机器装好依赖即可运行；`dsw/` 只是其中一种"环境预设"。

## 平台 × 任务矩阵

| 平台 | 训练 | 推理 | 环境怎么装 |
| --- | --- | --- | --- |
| 魔搭 DSW (A10 24G) | ❌ 卡不够 | `offline/infer.py` | `bash offline/dsw/setup.sh` |
| **实体 GPU / 任意 CUDA 机** | 视显存 | `offline/infer.py` | `pip install -r requirements-lock.txt`，**零额外脚本** |
| （未来离线平台 X） | ? | `offline/infer.py` | 往本目录加一个 `x/setup.sh` 预设即可 |
...
见 [`dsw/`](dsw/)：`setup.sh`（阿里云镜像 + 与云端 `MODAL_GPU_PACKAGES` 同版本锁定的环境安装）
和 `run.sh`（一键推理 + 提交打包）。
